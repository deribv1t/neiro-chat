...
def return_neiro(text):
    llm = Llama(
    model_path=os.path.join("..","..", "Model", "DeepSeek-R1-Distill-Llama-8B-Q4_K_M.gguf"),
    n_ctx=2048,
    n_threads=8,
    verbose=False,
    temperature=0.7
    ...
    for chunk in stream:
        ...
        if "content" in delta:
            ...
            elif ('<think>' in label["text"]):
                label["text"] = label["text"].replace('<think>\n','')
                label["font"] = ('Audiowide',10)
                label["background"] = '#1c344a'
                label["foreground"] = "#808080"

            elif ('</think>' in label["text"]):
                label["text"] = label["text"].replace('\n<think>\n','')
                label["font"] = ('Open Sans',12)
                label["background"] = '#2b5378'
